...
const char* getLibVersion() throw()
{
    return sqlite3_libversion();
}

// Return SQLite version number using runtime call to the compiled library
int getLibVersionNumber() throw()
{
...
{
	sqlite3* handle;
	const int ret = sqlite3_open_v2(aFilename.c_str(), 
        &handle, aFlags, aVfs.empty() ? NULL : aVfs.c_str());
...
long long Database::getLastInsertRowid() const throw ()
{
...
int Database::getTotalChanges() const throw ()
{
    return sqlite3_total_changes(getHandle());
}

// Return the numeric result code for the most recent failed API call (if any).
int Database::getErrorCode() const throw ()
{
    return sqlite3_errcode(getHandle());
}

// Return the extended numeric result code for the most recent failed API call (if any).
int Database::getExtendedErrorCode() const throw ()
{
    return sqlite3_extended_errcode(getHandle());
}

// Return UTF-8 encoded English language explanation of the most recent failed API call (if any).
const char* Database::getErrorMsg() const throw ()
{
