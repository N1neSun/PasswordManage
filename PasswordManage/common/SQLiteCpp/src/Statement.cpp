...
Statement::Statement(Statement&& aStatement) throw () :
...
int Statement::tryExecuteStep()  throw ()
{
    if (false == mbDone)
    {
...
int Statement::getBindParameterCount() const  throw ()
{
    return sqlite3_bind_parameter_count(mStmtPtr);
}

// Return the numeric result code for the most recent failed API call (if any).
int Statement::getErrorCode() const  throw ()
{
    return sqlite3_errcode(mStmtPtr);
}

// Return the extended numeric result code for the most recent failed API call (if any).
int Statement::getExtendedErrorCode() const  throw ()
{
    return sqlite3_extended_errcode(mStmtPtr);
}

// Return UTF-8 encoded English language explanation of the most recent failed API call (if any).
const char* Statement::getErrorMsg() const  throw ()
{
